...
if (!sourcePath) {
  console.log(
    '[storage-sync] STORAGE_SYNC_SOURCE_PATH não configurado e fonte padrão não encontrada; mantendo fonte atual.',
  );
  process.exit(0);
}
...
if (!existsSync(resolvedSourcePath)) {
  console.error(`[storage-sync] Arquivo não encontrado: ${resolvedSourcePath}`);
  process.exit(1);
}

if (!statSync(resolvedSourcePath).isFile()) {
  console.error(`[storage-sync] O caminho configurado não é um arquivo: ${resolvedSourcePath}`);
  process.exit(1);
}
...
if (!['.xlsx', '.xls', '.csv', '.xml'].includes(extension)) {
  console.error(
    `[storage-sync] Formato não suportado: ${extension}. Use .xlsx, .xls, .csv ou .xml.`,
  );
  process.exit(1);
}
