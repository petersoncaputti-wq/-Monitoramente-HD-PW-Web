<#
.SYNOPSIS
Exporta propriedades dos projetos do ProjectWise para o layout "Modelo de criacao de projeto".

.DESCRIPTION
O script busca Work Areas/Rich Projects no ProjectWise e gera um CSV com as mesmas colunas
da planilha modelo:
...
.EXAMPLE
.\scripts\Export-PWProjectCreationModel.ps1 -UseGui

.EXAMPLE
.\scripts\Export-PWProjectCreationModel.ps1 -DatasourceName "Servidor:Datasource" -BentleyIMS -NonAdminLogin

.EXAMPLE
.\scripts\Export-PWProjectCreationModel.ps1 -DatasourceName "Servidor:Datasource" -FolderPath "Projetos\Ecovias Araguaia" -OutputPath ".\exports\projetos.csv"

.EXAMPLE
.\scripts\Export-PWProjectCreationModel.ps1 -UseGui -InteractiveSelection
#>
...
function Get-RichProjectFromFolder {
    ...
    try {
        ...
        Write-Host "Aviso: nao foi possivel ler propriedades de '$folderPath' via Get-PWFolderPathAndProperties. Tentando Get-PWRichProjects."
    }
    ...
    try {
        ...
        Write-Host "Aviso: nao foi possivel ler propriedades de '$folderPath' via Get-PWRichProjects. Usando dados basicos da pasta."
    }
...
try {
    ...
    if ($InteractiveSelection -or [string]::IsNullOrWhiteSpace($FolderPath)) {
        Write-Host 'Buscando pasta de engenharia na raiz do datasource...'
        ...
        Write-Host "Buscando concessoes em: $(Get-DisplayName -Item $engineeringFolder)"
        ...
        Write-Host "Concessao selecionada: $(Get-DisplayName -Item $selectedConcession)"
        ...
        Write-Host "Buscando projetos em: $(Get-DisplayName -Item $projectsFolder)"
    ...
    if ($selectedProjectFolders) {
        Write-Host 'Lendo propriedades dos projetos selecionados...'
        $projects = @($selectedProjectFolders | ForEach-Object { Get-RichProjectFromFolder -Folder $_ })
    }
    else {
        Write-Host 'Buscando projetos no ProjectWise...'
    ...
    $rows = foreach ($project in $projects) {
        ...
        $nomeConcessao = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectNomeConcessao', 'NomeConcessao', 'Nome Concessao')
        if ([string]::IsNullOrWhiteSpace("$nomeConcessao")) {
            $nomeConcessao = Get-PathSegment -Path $projectPath -Index 1
        }

        $siglaConcessao = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectNovaSiglaANTT', 'ProjectSiglaDaConcesso', 'Nova Sigla')
        ...
        $row = [ordered]@{
            ...
            'Nome Projeto' = Get-ObjectPropertyValue -InputObject $project -Names @('Name')
            'Descricao' = Get-ObjectPropertyValue -InputObject $project -Names @('Description', 'Descripition')
            'Projetista' = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectNomeProjetista', 'Nome Projetista')
            'Gestor Engenharia' = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectGestorDaEngenharia', 'Gestor da Engenharia')
            'Assistente Engenharia' = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectAssistente', 'Assistente')
            'Poder Concedente' = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectPoderConcedente', 'P. Concedente')
            'TH' = Get-ProjectPropertyValue -PropertyMap $properties -Names @('ProjectTH', 'TH')
        }
