...
function EcoView({ data }: { data: EcoData }) {
  ...
  return <section id="eco-acompanhamento" className="mt-6 space-y-5">
    ...
    <section className={card} aria-label="Agenda consolidada">
      ...
      <Table headers={['Data', 'Unidade', 'Evento', 'Situação', 'Formato', 'Evidência']} rows={groupAgenda.map(item => [date(item.Data), item.Unidade, item.Evento, item.Situacao, item.Formato, item.Evidencia])} />
    </section>
      ...
      <section className={card} aria-label="Pendências e próximos passos">
        ...
        <div className="space-y-3">{visible.map(item => <details key={item.Objetivo} className="rounded-xl border border-brand-100 p-4"><summary className="cursor-pointer text-sm font-medium text-surface-900"><span>{item.Objetivo}</span><span className="ml-3 inline-block rounded-lg bg-brand-50 px-2 py-1 text-xs text-brand-700">{percent(item.Cumprimento)}</span><span className="ml-2 text-xs text-surface-600">{item.Situacao}</span></summary><div className="mt-3 space-y-2 text-sm text-surface-700"><p><strong>Categoria:</strong> {item.Categoria}</p><p><strong>Prazo registrado:</strong> {date(item.Prazo)}</p>{overdue(item, unit.DataReferencia) ? <p className="text-amber-800">Prazo anterior à referência, com cumprimento abaixo de 100%.</p> : normalize(item.Situacao) === 'prazo vencido' ? <p className="text-amber-800">“Prazo vencido” é a situação informada na planilha.</p> : null}</div></details>)}</div>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-2">
        <section className={card} aria-label="Agenda da unidade"><h4 className="text-lg font-semibold text-surface-900">Agenda a partir de {date(unit.DataReferencia)}</h4><p className="mt-1 mb-4 text-sm text-surface-600">Reuniões agendadas e marcos futuros, com a situação registrada na fonte.</p>{!upcoming.some(item => item.kind === 'Reunião') && <p className="mb-3 text-sm text-surface-600">Nenhuma próxima reunião registrada para esta unidade.</p>}<Table headers={['Data', 'Evento', 'Situação', 'Formato', 'Evidência']} rows={upcoming.map(item => [date(item.Data), item.Evento, item.Situacao, item.Formato, item.Evidencia])} /></section>
        ...
        {calculatedPresence !== null && Math.round(calculatedPresence * 100) !== Math.round(unit.PresencaMedia * 100) && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">Conferir presença: média das reuniões apuradas {new Intl.NumberFormat('pt-BR', { style: 'percent', maximumFractionDigits: 2 }).format(calculatedPresence)}; indicador informado na planilha {percent(unit.PresencaMedia)}. O valor da fonte foi preservado.</p>}<details className="mt-5 border-t border-brand-100 pt-4"><summary className="cursor-pointer text-sm font-semibold text-brand-700">Histórico de reuniões ({history.length})</summary><div className="mt-3"><Table headers={['Data', 'Tipo', 'Formato', 'Evidência', 'Status', 'Presentes / convocados', 'Participação']} rows={history.map(item => [date(item.Data), item.Tipo, item.Formato || 'Não informado', item.Evidencia || item.Fonte, item.Status, `${item.Presentes ?? 'n/d'} / ${item.Convocados ?? 'n/d'}`, item.Convocados !== null && item.Convocados > 0 && item.Presentes !== null ? percent(item.Presentes / item.Convocados) : null])} /></div></details></section>
      </div>
