...
type Objective = typeof data.Objetivos[number];
...
export function KartadoEcoPage() {
  ...
  const upcoming = meetings.filter(item => item.Status === 'Agendada' && item.Data >= unit.DataReferencia);
  const history = meetings.filter(item => !upcoming.includes(item));
  ...
  return <section id="eco-acompanhamento" className="mt-6 space-y-5">
    ...
    <section key={unitName} className="space-y-5" aria-label={`Acompanhamento de ${unitName}`}>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-2">
        <section className={card} aria-label="Agenda da unidade"><h4 className="text-lg font-semibold text-surface-900">Agenda a partir de {date(unit.DataReferencia)}</h4><p className="mt-1 mb-4 text-sm text-surface-600">Reuniões e marcos registrados como agendados na fonte.</p><Table headers={['Data', 'Tipo', 'Fonte']} rows={upcoming.map(item => [date(item.Data), item.Tipo, item.Fonte])} /></section>
        ...
        <details className="mt-5 border-t border-brand-100 pt-4"><summary className="cursor-pointer text-sm font-semibold text-brand-700">Histórico de reuniões e marcos ({history.length})</summary><div className="mt-3"><Table headers={['Data', 'Tipo', 'Status', 'Presentes / convocados', 'Participação']} rows={history.map(item => [date(item.Data), item.Tipo, item.Status, `${item.Presentes ?? 'n/d'} / ${item.Convocados ?? 'n/d'}`, item.Convocados !== null && item.Convocados > 0 && item.Presentes !== null ? percent(item.Presentes / item.Convocados) : null])} /></div></details></section>
      </div>
