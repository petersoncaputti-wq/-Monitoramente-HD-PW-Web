...
Formato v2: aceita a aba opcional Marcos (Unidade, Data, Marco, Situacao, Fonte), além de Formato e Evidencia em Reunioes. Evidencia substitui Fonte quando esta não existe. Importações e arquivos v1 continuam válidos. A agenda une reuniões e marcos, preservando situação e descrição; quando Marcos existe, registros legados Tipo=Marco não são repetidos. Marcos concluídos/cancelados ficam no histórico. A prévia lista objetivos ausentes ou renomeados em relação à base carregada. A interface sinaliza divergência de arredondamento entre presença informada e média das reuniões elegíveis sem sobrescrever o indicador. Não requer migração de banco: os campos são salvos em JSONB.

Gráficos: cumprimento publicado por unidade (0–100%); barras empilhadas com escala absoluta comum e grupos exclusivos (100%, Parado, Aguardando, Não iniciado/zero, outros pendentes); barras por objetivo dentro dos detalhes filtráveis; participação por reunião elegível com ausência de apuração distinta de zero. A escala de participação comporta valores acima de 100% sem cortar. Marcos futuros aparecem em linha do tempo, sem repetição na tabela de reuniões. Nenhum gráfico infere evolução histórica.
